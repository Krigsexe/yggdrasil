...
import { YggdrasilResponse, EpistemicBranch } from "@/lib/yggdrasil/types"
import React from "react"
import { toast } from "sonner"
import { v4 as uuidv4 } from "uuid"

/** Result from processing a response including YGGDRASIL metadata */
export interface ProcessResponseResult {
  text: string
  yggdrasilResponse?: Partial<YggdrasilResponse>
}
...
export const handleHostedChat = async (
  ...
  // Route through YGGDRASIL if enabled (default), otherwise use direct provider
  const useYggdrasil = payload.chatSettings.yggdrasilMode ?? true
  const apiEndpoint = useYggdrasil
    ? "/api/chat/yggdrasil"
    : `/api/chat/${provider}`
...
export const processResponse = async (
  ...
  if (response.body) {
    ...
    // Update answer in YGGDRASIL response
    if (yggdrasilResponse) {
      yggdrasilResponse.answer = fullText
    }

    return { text: fullText, yggdrasilResponse }
  } else {
